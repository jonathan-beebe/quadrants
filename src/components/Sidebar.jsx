...
export default function Sidebar({
  ...
  return (
    <>
      <aside className={`fixed top-0 left-0 w-[280px] h-screen bg-surface border-r border-border flex flex-col z-[100] transition-transform duration-150 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between p-4 border-b border-border">
          ...
          <div className="flex items-center gap-1">
            <button className="btn-icon text-text-secondary" onClick={onToggleDark} title="Toggle dark mode">
              {darkMode ? (
            ...
            <button className="btn-icon text-text-secondary" onClick={onToggle} title="Toggle sidebar">
        ...
        <div className="px-4 py-3 flex flex-col gap-1.5">
          ...
          <button className="btn-ghost btn-sm" onClick={onImport} title="Import JSON">
        ...
        <div className="flex-1 overflow-y-auto px-2 py-1">
          {frameworks.length === 0 && (
            <div className="py-6 px-4 text-center text-text-tertiary text-[13px]">No frameworks yet</div>
          )}
          {frameworks.map((fw) => (
            <div
              key={fw.id}
            ...
            >
              ...
              {menuId === fw.id && (
                <div className="absolute right-2 top-full bg-surface border border-border rounded-lg shadow-lg z-[200] min-w-[140px] p-1" ref={menuRef}>
                  <button
                    className="block w-full text-left px-3 py-2 text-[13px] rounded text-text hover:bg-bg"
                    onClick={(e) => { e.stopPropagation(); onDuplicate(fw); setMenuId(null) }}
                  >
                    Duplicate
                  </button>
                  <button
                    className="block w-full text-left px-3 py-2 text-[13px] rounded text-text hover:bg-bg"
                    onClick={(e) => { e.stopPropagation(); onExport(fw); setMenuId(null) }}
                  >
                    Export JSON
                  </button>
                  <button
                    className="block w-full text-left px-3 py-2 text-[13px] rounded text-danger hover:bg-red-50 dark:hover:bg-red-950"
                    onClick={(e) => { e.stopPropagation(); onDelete(fw.id); setMenuId(null) }}
                  >
                    Delete
                  </button>
                </div>
